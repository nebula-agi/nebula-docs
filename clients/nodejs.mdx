...
const client = new NebulaClient({ apiKey: 'your_api_key_here' });
```
...
(async () => {
  ...
  const memory = await client.store({ content: 'Hello world', clusterId: 'cluster_123' });
})();
```
...
```javascript
const memory = await client.get('memory_id_here');
```
...
**Returns:** Array of `Memory` objects with content retrieved from chunks
