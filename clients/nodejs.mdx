...
**Parameters:**
- `cluster_id` (string): ID of the cluster to retrieve
...
**Parameters:**
- `cluster_id` (string): ID of the cluster to update
...
The SDK supports two ways to provide memory data:

#### Option 1: Plain Objects (Recommended)
```javascript
const memoryId = await client.storeMemory({
...
#### Option 2: Memory Interface Objects
```javascript
const memory = {
  cluster_id: 'user_preferences',
  content: 'User prefers email communication over phone calls',
...
Both approaches work identically and accept the same parameters: `cluster_id`, `content`, `role`, `parent_id`, and `metadata`.
...
**Parameters:**
- `cluster_ids` (array): Array of cluster IDs to retrieve memories from (required)
...
**Parameters:**
- `cluster_id` (string): ID of the cluster (required)
- `limit` (number): Maximum number of memories to return
- `offset` (number): Number of memories to skip
...
- `cluster_ids` (array): Clusters to search within (required)
...
(async () => {
  ...
  const clusterMemories = await client.listMemories({ cluster_ids: [cluster.id] });
