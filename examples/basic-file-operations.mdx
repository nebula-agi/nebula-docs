...
<Tabs>
  ...
  <Tab title="Node.js">
    ...
    async function uploadMultipleFiles(clusterId, filePaths, showProgress = true) {
      ...
      for (let i = 0; i < filePaths.length; i++) {
        ...
        if (showProgress) {
          process.stdout.write(`\r📤 Uploading ${fileName} (${i + 1}/${filePaths.length})`);
        }
        ...
        try {
          ...
          if (showProgress) {
            process.stdout.write(`\r❌ Failed: ${fileName}`);
          }
        }
      }
  ...
  <Tab title="Go">
    ...
    func uploadMultipleFiles(clusterID string, filePaths []string, showProgress bool) ([]UploadResult, []UploadError) {
        ...
        fmt.Printf("📤 Starting upload of %d files...\n", len(filePaths))
        ...
        for i, filePath := range filePaths {
            ...
            go func(path string, index int) {
                ...
                if showProgress {
                    fmt.Printf("\r📤 Uploading %s (%d/%d)", fileName, index+1, len(filePaths))
                }
...
<Tabs>
  ...
  <Tab title="Go">
    ...
    func downloadFolderContents(clusterID, folderID, outputDir string) ([]DownloadedFile, []DownloadError) {
        ...
        fmt.Printf("📁 Found %d items in folder\n", len(files))
...
<Tabs>
  <Tab title="Python">
    ...
    def create_organized_structure(cluster_id):
        ...
        for main_folder, sub_folders in folders.items():
            try:
                ...
                print(f"📁 Created main folder: {main_folder.title()}")
                ...
                for sub_key, sub_name in sub_folders.items():
                    ...
                    print(f"   📁 Created sub-folder: {sub_name}")
    ...
    print(f"\n📋 Folder structure created: {len(folder_structure)} folders")
    ```
  </Tab>
  <Tab title="Node.js">
    ```javascript
    async function createOrganizedStructure(clusterId) {
      ...
      for (const [mainFolder, subFolders] of Object.entries(folders)) {
        try {
          ...
          console.log(`📁 Created main folder: ${mainFolder.charAt(0).toUpperCase() + mainFolder.slice(1)}`);
          ...
          for (const [subKey, subName] of Object.entries(subFolders)) {
            ...
            console.log(`   📁 Created sub-folder: ${subName}`);
          }
    ...
    createOrganizedStructure('your-cluster-id')
      .then(folderStructure => {
        console.log(`\n📋 Folder structure created: ${Object.keys(folderStructure).length} folders`);
      })
      .catch(error => {
  ...
  <Tab title="Go">
    ...
    func createOrganizedStructure(clusterID string) map[string]string {
        ...
        for mainFolder, subFolders := range folders {
            ...
            createdFolders[mainFolder] = mainFolderObj.ID
            fmt.Printf("📁 Created main folder: %s\n", mainFolderName)
            ...
            for subKey, subName := range subFolders {
                ...
                fmt.Printf("   📁 Created sub-folder: %s\n", subName)
            }
        }
    ...
    fmt.Printf("\n📋 Folder structure created: %d folders\n", len(folderStructure))
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Node.js">
    ...
    async function organizeFilesByType(clusterId, folderId) {
      ...
      console.log(`   ❌ Failed: ${failedMoves.length}`);
