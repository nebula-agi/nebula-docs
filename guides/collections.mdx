...
<Note>Collection names are case-insensitive and trimmed. You cannot create two collections with the same name (e.g., "Work" and "work" are treated as duplicates).</Note>
...
# Attempting to create "work" (lowercase) will fail if "Work" already exists
try:
    nebula.create_collection(name="work", description="Duplicate")
except Exception as e:
    print(f"Error: {e}")  # Collection with name 'work' already exists
```
...
async function createCollections() {
  ...
  const collections = await Promise.all([
    nebula.createCollection({ name: "Work", description: "Work-related notes" }),
    nebula.createCollection({ name: "Personal", description: "Personal memories" })
  ]);
}
```
...
```javascript JavaScript
// Store in specific collection
const workMemory = await nebula.storeMemory({
...
WORK_COLLECTION_ID=$(curl -X GET "https://api.nebulacloud.app/v1/collections?name=Work" \
  -H "Authorization: Bearer YOUR_API_KEY" | jq -r '.results[0].id')

# Store in specific collection using collection_id
curl -X POST "https://api.nebulacloud.app/v1/memories" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d "{
    \"collection_id\": \"$WORK_COLLECTION_ID\",
    \"engram_type\": \"document\",
    \"raw_text\": \"Q4 planning meeting notes\",
    \"metadata\": {\"type\": \"meeting\", \"date\": \"2024-01-15\"}
  }"

# Get Personal collection ID
PERSONAL_COLLECTION_ID=$(curl -X GET "https://api.nebulacloud.app/v1/collections?name=Personal" \
  -H "Authorization: Bearer YOUR_API_KEY" | jq -r '.results[0].id')

# Store in another collection
curl -X POST "https://api.nebulacloud.app/v1/memories" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d "{
    ...
    \"engram_type\": \"document\",
    \"raw_text\": \"Book recommendation: Clean Code\",
    \"metadata\": {\"category\": \"books\"}
  }"
```
...
```javascript JavaScript
// Search by collection name
const workResults = await nebula.search({
  query: "project deadlines",
  collection_ids: ["Work"]
});
...
// Search multiple collections (mix names and IDs)
const allResults = await nebula.search({
  query: "important notes",
  collection_ids: ["Work", personalCollection.id]
});

// Search all accessible collections
const globalResults = await nebula.search({
  query: "important notes"
});
...
<Tabs>
  <Tab title="List Collections">
    <CodeGroup>
      ...
      # Filter by name (case-insensitive)
      curl -X GET "https://api.nebulacloud.app/v1/collections?name=Work" \
        -H "Authorization: Bearer YOUR_API_KEY"
      ```
    </CodeGroup>
  </Tab>
  ...
  <Tab title="Update Collection">
    <CodeGroup>
      ```python Python
      # Update collection
      updated = nebula.update_collection(
      ...
      ```javascript JavaScript
      // Update collection
      const updated = await nebula.updateCollection({
