...
**Batch retrieval (JavaScript only):**
```javascript batch_history.js
const { NebulaClient } = require('@nebula-ai/sdk');

const client = new NebulaClient({ apiKey: 'your-api-key' });

async function getBatchConversationHistory() {
  // Get multiple conversations at once for better performance
  ...
  // Returns a dictionary mapping conversation IDs to their messages
  ...
  for (const [convId, messages] of Object.entries(batchResults)) {
    ...
    messages.forEach(message => {
      const role = message.metadata.source_role || message.metadata.role || "unknown";
      console.log(`  [${role.toUpperCase()}]: ${message.content.substring(0, 60)}...`);
    });
  }
}

getBatchConversationHistory();
```
