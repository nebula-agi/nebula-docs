...
nebula.store_memory(Memory(
    collection_id='my-collection',
    content=[DocumentContent(data=pdf_data, media_type='application/pdf')],
    fast_mode=True,  # Uses pypdf for instant extraction
))
```
...
await nebula.storeMemory({
  ...
  fast_mode: false,  // Default: uses VLM OCR
  vision_model: 'gpt-4o',  // Custom vision model
});
...
|--------|---------|-------------|
| `fast_mode` | `false` | `false`: VLM OCR (best quality), `true`: fast pypdf extraction |
...
<Tip>Use `fast_mode=False` (default) for scanned documents and image-heavy PDFs. Use `fast_mode=True` for text-based PDFs when you need instant results.</Tip>
...
5. **Choose the right multimodal mode** - VLM for scanned docs, fast mode for text PDFs
