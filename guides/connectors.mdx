...
<Tabs>
  <Tab title="Google Drive Folders">
    ...
    folders.forEach(f => console.log(f.id, f.name, f.is_selected));
    ```
    ...
    curl -X PATCH "https://api.trynebula.ai/v1/connectors/CONNECTION_ID/config" \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -d '{"config": {"folder_ids": ["1ABC...", "2DEF..."]}, "apply": "full_resync"}'
    ```
    </CodeGroup>
  </Tab>
  ...
  <Tab title="Slack Channels">
    ...
    channels.forEach(ch => console.log(ch.id, ch.name, ch.num_members, ch.is_selected));
    ```
    ...
    curl -X PATCH "https://api.trynebula.ai/v1/connectors/CONNECTION_ID/config" \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -d '{"config": {"channel_ids": ["C01234ABC", "C56789DEF"]}, "apply": "full_resync"}'
    ```
    </CodeGroup>
  </Tab>
</Tabs>
...
connections.forEach(conn => console.log(conn.provider, conn.status, conn.health));
```
...
```javascript JavaScript
const conn = await client.getConnection('CONNECTION_ID');
console.log(conn.health, conn.next_sync_at);
```

```bash cURL
curl -X GET "https://api.trynebula.ai/v1/connectors/CONNECTION_ID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
</CodeGroup>
...
curl -X PATCH "https://api.trynebula.ai/v1/connectors/CONNECTION_ID/config" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{"config": {"folder_ids": ["NEW_FOLDER_ID"]}, "apply": "full_resync"}'
```
</CodeGroup>
...
// Disconnect and delete synced memories
const result = await client.disconnect('CONNECTION_ID', true);
console.log(result.warnings); // [] on success
```
...
# Disconnect and delete synced memories
curl -X DELETE "https://api.trynebula.ai/v1/connectors/CONNECTION_ID?delete_memories=true" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
</CodeGroup>
...
<Warning>Memory deletion is best-effort. Memories created before connection tracking was added,
...
cleanup, delete the entire collection.</Warning>
...
```javascript JavaScript
const result = await client.triggerSync('CONNECTION_ID');
console.log(result.message); // "Sync triggered"
```

```bash cURL
curl -X POST "https://api.trynebula.ai/v1/connectors/CONNECTION_ID/sync" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
</CodeGroup>
...
<Note>`/folders` and `/channels` are GET endpoints but require EDIT permission because they access the provider's API using stored credentials.</Note>
...
- [Search](/guides/search) - Search across synced content
