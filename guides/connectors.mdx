...
<Tip>Scope connections narrowly by selecting specific folders or channels. Each collection supports one active connection per provider and account. Use the folder/channel browsing APIs to discover IDs programmatically, and monitor `last_synced_at` and `last_error` fields for sync health.</Tip>
