...
# List memories
curl -X GET "https://api.nebulacloud.app/v1/engrams?limit=50" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash cURL
curl -G "https://api.nebulacloud.app/v1/memories" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  --data-urlencode 'collection_ids=docs' \
  --data-urlencode 'limit=1000' \
  --data-urlencode 'metadata_filters={"metadata.status":{"$eq":"archived"}}'

curl -X POST "https://api.nebulacloud.app/v1/memories/delete" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
