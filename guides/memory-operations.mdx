...
Upload a document as raw text, pre-split text, or a file. File processing (OCR/transcription/text extraction) happens automatically.
...
```javascript JavaScript
const memory = await nebula.getMemory(memoryId);
memory.sources?.forEach(source => console.log(`Source ${source.id}: ${source.content}`));

await nebula.deleteSource(sourceId);
await nebula.updateSource(sourceId, "Updated content", { edited: true });
```
...
# Delete source
curl -X DELETE "https://api.trynebula.ai/v1/sources/YOUR_SOURCE_ID" \
  -H "Authorization: Bearer YOUR_API_KEY"

# Update source
curl -X PATCH "https://api.trynebula.ai/v1/sources/YOUR_SOURCE_ID" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
