...
Filter search results by metadata attributes using MongoDB-like operators. Supports comprehensive filtering for both vector/chunk search and graph search (entities, relationships, communities).
...
results = client.search(
    ...
    filters={
        "metadata.skills": {"$overlap": ["python", "tensorflow", "pytorch"]}
    },
    limit=10
)
...
<Note>
For comprehensive metadata filtering documentation including all supported operators, nested paths, and real-world examples, see the [Metadata Filtering Guide](/guides/metadata-filtering).

**Supported operators:** `$eq`, `$ne`, `$gt`, `$gte`, `$lt`, `$lte`, `$like`, `$ilike`, `$in`, `$nin`, `$overlap`, `$contains`, `$and`, `$or`
</Note>

**Advanced search settings:**

Customize search behavior with semantic/fulltext weights and graph-specific filters:
...
# Control semantic vs full-text balance with graph filtering
results = client.search(
    query="research methodology",
    cluster_ids=["research-cluster"],
    search_settings={
        "limit": 15,
        "semantic_weight": 6.0,      # Emphasize conceptual matching
