...
memory_id = nebula.store_memory(
    Memory(collection_id="research", content="Machine learning automates model building", metadata={"topic": "AI"})
)

# Batch storage
memory_ids = nebula.store_memories([
    Memory(collection_id="research", content="Neural networks...", metadata={"type": "concept"}),
    Memory(collection_id="research", content="Deep learning...", metadata={"type": "concept"})
])
```
...
<Tip>
Python's `store_memory()` also accepts a plain dict: `nebula.store_memory({"collection_id": "...", "content": "..."})`. Similarly, JavaScript's `storeMemory()` accepts a `Memory()` wrapper: `nebula.storeMemory(Memory({ ... }))`. Both forms are equivalent -- use whichever you prefer.
</Tip>
...
conv_id = nebula.store_memory(
    Memory(collection_id="support", content="Hello! How can I help?", role="assistant")
)

# Add to same conversation
nebula.store_memory(
    Memory(memory_id=conv_id, collection_id="support", content="I need help with my account", role="user")
)
```
...
```javascript JavaScript
// Batch store documents
const ids = await nebula.storeMemories([
  { collection_id: collection.id, content: "First document" },
  { collection_id: collection.id, content: "Second document" },
  { collection_id: collection.id, content: "Third document" },
]);

// Batch store conversation messages (grouped into one conversation)
const convIds = await nebula.storeMemories([
  { collection_id: collection.id, content: "Hello!", role: "user" },
  { collection_id: collection.id, content: "Hi there!", role: "assistant" },
]);
```
...
```bash cURL
# Get by ID
curl -X GET "https://api.trynebula.ai/v1/memories/YOUR_MEMORY_ID" \
  -H "Authorization: Bearer YOUR_API_KEY"

# List memories
curl -X GET "https://api.trynebula.ai/v1/memories?limit=50" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash cURL
curl -X DELETE "https://api.trynebula.ai/v1/memories/YOUR_MEMORY_ID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
# Update chunk
curl -X PATCH "https://api.trynebula.ai/v1/chunks/YOUR_CHUNK_ID" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
