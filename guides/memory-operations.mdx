...
const ids = await nebula.storeMemories([
  { collection_id: collection.id, content: "First document", metadata: {} },
  { collection_id: collection.id, content: "Second document", metadata: {} },
  { collection_id: collection.id, content: "Third document", metadata: {} },
]);

// Batch store conversation messages (grouped into one conversation)
const convIds = await nebula.storeMemories([
  { collection_id: collection.id, content: "Hello!", role: "user", metadata: {} },
  { collection_id: collection.id, content: "Hi there!", role: "assistant", metadata: {} },
]);
```
