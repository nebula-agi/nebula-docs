...
<Tip>Use `store_memories()` for batch operations -- it's faster than multiple single calls. Rich metadata improves search quality; see [Metadata Filtering](/guides/metadata-filtering) for filter operators.</Tip>
