...
curl -X POST "https://api.nebulacloud.app/v1/memories" \
  ...
  -d '{
    "collection_ref": "research-collection",
...
nebula.store_memory(Memory(
    ...
    fast_mode=True  # Optional: use fast text extraction instead of VLM OCR
))
```
...
|--------|------|---------|-------------|
| `fast_mode` | bool | `false` | Use fast text extraction for PDFs (pypdf) instead of VLM OCR |
| `vision_model` | string | auto | Vision model for images/documents |
| `audio_model` | string | `whisper-1` | Audio transcription model |

<Tip>Use `fast_mode=True` for quick text extraction from text-based PDFs. Default (`false`) uses VLM OCR for best quality on scanned documents.</Tip>
