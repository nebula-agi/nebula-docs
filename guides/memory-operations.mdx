...
Store images, audio, and documents. Nebula automatically processes them using state-of-the-art multimodal models.
...
# 1. Store a single file (Simplest)
nebula.store_memory(Memory.from_file(
    "paris.jpg", 
    collection_id='my-collection',
    metadata={'type': 'photo'}
))

# 2. Mixed content (Text + Image)
nebula.store_memory(Memory(
    collection_id='my-collection',
    content=[
        'Photo from my trip',
        Memory.File('paris.jpg')
    ],
    metadata={'type': 'photo'}
))
```
...
// 1. Store a single file (Simplest)
await nebula.storeMemory(await Memory.fromFile(
  'paris.jpg', 
  'my-collection',
  { type: 'photo' }
));
...
curl -X POST "https://api.nebulacloud.app/v1/memories" \
  ...
  -d '{
    ...
    "engram_type": "document",
    "content_parts": [
      {"type": "file", "data": "BASE64_PDF_DATA", "media_type": "application/pdf"}
    ],
