...
- **Knowledge**: Subject-predicate-value assertions (e.g., "Sarah - led - the Aurora migration"), with confidence that grows through corroboration across sources
...
4. **Retrieval**: `search()` returns knowledge, procedures, episodes, and sources; `get_memory()` returns the raw memory with all chunks
