---
title: "Core Concepts"
description: "Understanding Nebula's architecture: memories, sources, and the vector graph"
---
...
- **Document Memory**: An entire document, automatically split into sources for processing. You can upload text, pre-split text, or a file. A short piece of text (like a user preference) is a document memory with a single source.
...
- **Sources**: The original source text that grounds each entity and fact, providing provenance back to the stored memory
...
4. **Retrieval**: `search()` returns entities, facts, and sources; `get_memory()` returns the raw memory with all sources
