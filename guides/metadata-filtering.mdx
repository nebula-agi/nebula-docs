...
> Metadata in responses: By default, Nebula returns whitelisted metadata in search and MemoryRecall results (`include_metadata` defaults to true). Set `include_metadata: false` to omit metadata entirely. The whitelist for recall includes common safe fields like `title`, `source`, `url`, `doc_type`, `mime_type`, `filename`, and `page`.
