...
# Knowledge: structured subject-predicate-value triples
for fact in results.knowledge:
...
# Sources: original source text grounding the knowledge
...
|-------|----------|---------|
| **Knowledge** | Subject-predicate-value triples (facts, inferences, tasks) | `Sarah Chen → led → Aurora migration` |
...
| **Sources** | Original source text that grounds the knowledge | `"Sarah led the migration from PostgreSQL to Aurora last quarter"` |

Each item has an `activation_score` (0-1) reflecting its relevance to the query. Knowledge items link back to their source text, giving you a full provenance chain from structured assertions down to original content.
