---
title: 'Search Guide'
description: 'Knowledge graph search in Nebula'
---

Nebula searches over a knowledge graph of entities and relationships, not raw text chunks. You send a natural language query; Nebula traverses the graph and returns structured knowledge: **entities** (who and what), **facts** (assertions about them), and **utterances** (the original source text).
...
<Info>The knowledge graph is built automatically when you store memories. You don't need to define entities or relationships - Nebula extracts them.</Info>

## Search Effort

The `effort` parameter is the primary control for search. It determines how deeply Nebula traverses the knowledge graph.
...
Use `collection_ids` to scope which part of the knowledge graph is searched.
