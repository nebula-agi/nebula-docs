...
Combine semantic search with metadata filtering. Simple key-value filters use equality matching, while operators like `$in`, `$gte`, and `$ne` handle more complex conditions.
...
<Tip>Use natural language queries for better results. Target specific collections for faster responses, and start with default weights before tuning.</Tip>
