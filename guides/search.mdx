...
Nebula searches over a vector graph of entities and relationships, not raw text chunks. You send a natural language query; Nebula traverses the graph and returns structured knowledge: **entities** (who and what), **facts** (assertions about them), and **sources** (the original source text).
...
# Sources: original source text grounding the facts
...
results.sources.forEach(s =>
  console.log(`[${s.speaker ?? s.source_role ?? 'unknown'}]: ${s.text}`)
);
```
...
| **Sources** | Original source text that grounds the facts | `"Sarah led the migration from PostgreSQL to Aurora last quarter"` |

Each item has an `activation_score` (0-1) reflecting its relevance to the query. Facts link back to their sources, and entities link to their facts - giving you a full provenance chain from structured knowledge down to original text.
