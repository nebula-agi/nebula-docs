...
Messages are sent to Nebula as conversation memories using `storeMemories()`. Nebula handles chunking, embedding, ranking, deduplication, and vector graph extraction server-side. The plugin tracks what has already been sent per session so nothing is duplicated.
...
```bash
npx skills add nebula-agi/skills
```

This installs the Nebula skill and wires up the hooks automatically.
