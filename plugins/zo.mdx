...
Long-term memory for [Zo](https://zo.computer) powered by Nebula. Stores conversations, decisions, preferences, and project context so Zo can recall them across sessions. Stored content feeds into a [vector graph](/guides/concepts#the-vector-graph) that returns entities, facts, and sources.
